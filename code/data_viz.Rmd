...
nutrition_map <- leaflet(data = veg_sf, options = leafletOptions(zoomControl = FALSE), height = 500, width = 450) %>% 
 #  addProviderTiles("CartoDB.Positron") %>% 
  ...
  addLegend(position ="topleft", 
            pal = pal, 
            values = veg_sf$pct, 
            title = "Proportion of<br>respondents<br>that had no<br>fruits/vegetables<br>the
            prior day", 
...
#### Map 2: Proportion of respondents that sometimes/often did not have enough food by UHF 
...
food_insc <- leaflet(data = fi_sf, options = leafletOptions(zoomControl = FALSE), height = 600, width = 500) %>% 
 #  addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(weight = .5, 
              fillColor = ~pal(pct), 
              color = "white", 
...
pal = colorNumeric(
 ...
 povmap <-  leaflet(data = pov_sf, options = leafletOptions(zoomControl = FALSE), height = 600, width = 500) %>% 
  # addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(weight = .5, 
  ...
  addLegend(position ="topleft", 
            ...
            title = "Proportion of<br> of adults below<br>poverty line", 
            labFormat = labelFormat(suffix = "%")
  )
...
pal = colorNumeric(
 ...
 racemap <- leaflet(data = race_sf, options = leafletOptions(zoomControl = FALSE), height = 500, width = 450) %>% 
  # addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(weight = .5, 
...
snap_ct <- leaflet(data = snap_nyc_sf, options = leafletOptions(zoomControl = FALSE), height = 600, width = 500) %>% 
  # addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(weight = .5, 
...
green_sp <- leaflet(data = vc_sf, options = leafletOptions(zoomControl = FALSE), height = 600, width = 500) %>% 
  # addProviderTiles("CartoDB.Positron") %>% 
  addPolygons(weight = .5, 
              fillColor = ~pal(Percent), 
              color = "white", 
...
ggplot(snap_time, aes(x = yr_mo, y = total_snap_recipients)) + geom_smooth(se=FALSE, color = "seagreen") + ylab("Total SNAP Recipients") + xlab("Year") + scale_y_continuous(labels = scales::number_format(big.mark = ",")) + ggtitle("SNAP Participation Over Time") + theme_bw()
ggsave("snap_participation.jpg", dpi = 1200)
```
...
### Population Growth Historical and Future Trends for Seniors 
...
# make ggplot 
p <- ggplot(aging_long[Year<=2010, ], aes(x=Year, y=Population, group=County, color=County)) + geom_line(size = 2) + geom_line(data = aging_long[Year>=2010, ], linetype="dashed") + scale_y_continuous(labels = scales::number_format(big.mark = ","), breaks = seq(from = 0, to=2000000, by = 100000)) + theme_minimal() +
...
ggsave("pop_growth_seniors.jpg", dpi = 1200, width = 5.5)
```
...
ggsave("seniors_pov.jpeg", dpi = 1200, width = 5.5)
```

### School Lunch 
```{r echo=FALSE, include=FALSE, warning=FALSE,message=FALSE, include=FALSE}
library(tidyverse)
lunch <- tribble (
...
```
### Pell Grants 
Citation: A Profile of Undergraduates at CUNY Senior and Community Colleges: Fall 2017 https://www2.cuny.edu/wp-content/uploads/sites/4/page-assets/about/administration/offices/oira/institutional/data/current-student-data-book-by-subject/ug_student_profile_f17.pdf
```{r echo=FALSE, warning=FALSE}
...
bp + xlab("") + ggtitle("Percent of CUNY Students That Received Pell Assistance in 2017") 
ggsave("pellgrants.jpg")
```

### School Lunch 
Community Food Advocates, Universal School Lunch in NYC Year One, Insights from High School Cafeterias, (March 2019), http://nyf.issuelab.org/resources/34401/34401.pdf. 
```{r echo=FALSE, warning=FALSE}
...
bp + labs(title = "School Lunch Participation Pre and Post Universal School Lunch",
       subtitle = "% Change in Participation 2016-2017",
       caption = "Source: Universal School Lunch in NYC Year One, Community Food Advocates") +
  theme(panel.grid = element_blank(),
        axis.title.y = element_blank(),
        axis.title.x = element_blank(),
...
ggsave("schoolfood.png", dpi = 1200, width = 6, height = 6)
```

### Food Waste
Wasted: How America is Losing Up To 40 Percent of its food Farm to Fork to Landfill, Second Edition of NRDC’s Original 2012 Report, National Resource Defense Council, (2017), https://www.nrdc.org/sites/default/files/wasted-2017-report.pdf. 
```{r echo=FALSE}
df <- data.frame(Percent = c(.54, .20, .05, .07, .04, .1), Class = c("Residential", "Restaurants or Caterers", "Grocers or Food Wholesale", "Food Manufacturing and Processing", "Health Care", "Other"))
bp<- ggplot(df, aes(x=reorder(Class, Percent), y=Percent, fill = Class)) +
geom_col(stat = "identity", show.legend = FALSE) + scale_y_continuous(labels = scales::percent)
bp + labs(title = "Origin of Food Waste in New York City",
       caption = "Source: NRDC Estimating Quantities and Types of Food Waste at the City Level") +
  theme(panel.grid = element_blank(),
        axis.title.y = element_blank(),
        axis.title.x = element_blank(),
...
ggsave("foodwaste.jpg",  dpi = 1200, width = 7.5, height = 6)

```


#### Heat Vulnerability Index 
New York City Department of Health and Mental Hygiene. Environment and Health Data Portal- Heat Vulnerability Index, 2010. 7/25/2019. http://nyc.gov/health/tracking
```{r echo=FALSE, warning=FALSE, message=FALSE}
cdsf <- read_sf("cd_sf")
vc <- fread("hvi.csv")
vc[, boro_cd := geo_entity_id]

vc_geom <- vc %>% left_join(cdsf, by = "boro_cd")
vc_sf <- vc_geom %>% st_as_sf() 
vc_sf <- st_transform(vc_sf, '+proj=longlat +datum=WGS84')

pal = colorNumeric(
  palette = "Reds",
  domain = as.integer(vc_sf$data_value), 
  na.color = "Grey", 
  reverse = FALSE
)

hvi <- leaflet(data = vc_sf, options = leafletOptions(zoomControl = FALSE), 
